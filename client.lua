    ...
    function(data, menu)
      ...
      if data.current.value == 'vehiclemenu' then
        ...
        if IsPedSittingInAnyVehicle(playerPed) then
          ...
          TriggerEvent("pNotify:SendNotification", {text = _U('must_in_vehicle'), type = "error", queue = "vehiclemenu", timeout = 5000, layout = "bottomLeft"})
        end
      end
              ...
              local playerPed = GetPlayerPed(-1)
                ...
                if (IsPedSittingInAnyVehicle(playerPed)) then
                  if GetVehicleDoorAngleRatio(playerVeh, 4) > 0.0 then 
                      ...
                      TriggerEvent("pNotify:SendNotification", {text = "Motorkap gesloten!", type = "error", queue = "vehiclemenu", timeout = 1000, layout = "bottomLeft"})
                   else
                      SetVehicleDoorOpen(playerVeh, 4, false)
                      frontleft = true
                      TriggerEvent("pNotify:SendNotification", {text = "Motorkap geopend!", type = "success", queue = "vehiclemenu", timeout = 1000, layout = "bottomLeft"})
                    end
                end
            end
